...
public class ClientEventHandler {
	...
	private void renderDirectionBar(PoseStack poseStack, Player player, HudData data) {
		...
		if (data.hasTarget()) {
			...
			if (drawBarReadout(poseStack, player, data.displayCoordinates, bar.centerX, readoutY,
		...
		if (facedWaypoint != null) {
			drawWaypointReadout(poseStack, player, facedWaypoint, bar.centerX, readoutY);
		}
	}
	 ...
	 */
	private void drawWaypointReadout(PoseStack poseStack, Player player, HudWaypoint waypoint, int centerX, int y) {
		...
		mc.font.drawShadow(poseStack, DOT_GLYPH, left, y, 0xFF000000 | (waypoint.color() & 0xFFFFFF));
		mc.font.drawShadow(poseStack, text, left + dotWidth, y, GuiTheme.TEXT_PRIMARY);
	}
	 ...
	 */
	private boolean drawBarReadout(PoseStack poseStack, Player player,
		...
		mc.font.drawShadow(poseStack, readout.toString(), centerX - mc.font.width(readout.toString()) / 2.0F, y, color);
		return true;
	}
