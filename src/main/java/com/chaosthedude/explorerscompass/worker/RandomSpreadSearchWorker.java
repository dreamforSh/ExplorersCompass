...
public class RandomSpreadSearchWorker extends StructureSearchWorker<RandomSpreadStructurePlacement> {
	...
	protected boolean doSample() {
		if (hasWork()) {
			...
			// Move on to the next cell of the ring. Both coordinates have to be reset to -length using the
			// length of the ring they are about to walk, otherwise x never reaches -length and the entire
			// western column of every ring goes unsampled.
			z++;
			if (z > length) {
				x++;
				if (x > length) {
