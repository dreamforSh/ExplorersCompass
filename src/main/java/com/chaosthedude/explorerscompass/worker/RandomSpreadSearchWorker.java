...
public class RandomSpreadSearchWorker extends GridStructureSearchWorker<RandomSpreadStructurePlacement> {
	...
	protected boolean canPlaceAt(ChunkPos chunkPos) {
		...
		// The generator, the random state and the seed this used to be handed separately are all held by
		// the level's structure state now, which is the one thing the check asks for
		return !exclusionZone.get().isPlacementForbidden(level.getChunkSource().getGeneratorState(), chunkPos.x, chunkPos.z);
	}
