...
final class PreviewMeshBuilder {
	...
	/** One buffer of a built model, and the memory it is held in until it has been handed to the graphics card. */
	static final class Layer implements AutoCloseable {
		...
		private Layer(Kind kind, ByteBufferBuilder memory, MeshData mesh) {
			this.kind = kind;
			this.memory = memory;
			this.mesh = mesh;
		}
		...
		int quads() {
			return mesh.drawState().vertexCount() / 4;
		}
	...
	/** A buffer in the making: the memory, and the builder writing vertices into it. */
	private static final class Sink implements AutoCloseable {
		...
		Sink(Kind kind, int bytes, VertexFormat format) {
			this.kind = kind;
			memory = new ByteBufferBuilder(bytes);
			builder = new BufferBuilder(memory, VertexFormat.Mode.QUADS, format);
		}

		/**
		 * Finishes the buffer into a layer, or frees it when nothing was ever written to it. A model
		 * that left its last vertex half written cannot be finished, and costs the whole buffer.
		 */
		Layer finish() {
			final MeshData mesh;
			try {
				...
				ExplorersCompass.LOGGER.warn("A structure preview buffer could not be finished and was dropped", e);
				memory.close();
				return null;
			}
			if (mesh == null) {
			...
			return new Layer(kind, memory, mesh);
		}

		/** Frees the memory without finishing, when the build is being thrown away. */
		@Override
		public void close() {
			memory.close();
		}
