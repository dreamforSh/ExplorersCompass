...
public final class WaypointMarkers {
	 ...
	 */
	public static void drawWithShadow(WaypointMarkerStyle style, int x, int top, int bottom, int color, int shadowColor) {
		draw(style, x + 1, top + 1, bottom + 1, shadowColor);
		draw(style, x, top, bottom, color);
	}

	public static void draw(WaypointMarkerStyle style, int x, int top, int bottom, int color) {
		switch (style) {
			case BOOKMARK:
				drawBookmark(x, top, bottom, color);
				break;
			case FLAG:
				drawFlag(x, top, bottom, color);
				break;
			case DIAMOND:
				drawDiamond(x, top, bottom, color);
				break;
			default:
				drawPin(x, top, bottom, color);
				break;
		}
	}
	 ...
	 */
	private static void drawBookmark(int x, int top, int bottom, int color) {
		final int tail = bottom - 2;
		RenderUtils.drawRect(x - 2, top, x + 3, tail - 2, color);
		RenderUtils.drawRect(x - 2, tail - 2, x, tail - 1, color);
		RenderUtils.drawRect(x + 1, tail - 2, x + 3, tail - 1, color);
		RenderUtils.drawRect(x - 2, tail - 1, x - 1, tail, color);
		RenderUtils.drawRect(x + 2, tail - 1, x + 3, tail, color);
	}

	/** A head three pixels square on a stem down to the bottom edge. */
	private static void drawPin(int x, int top, int bottom, int color) {
		final int head = Math.max(top, bottom - 8);
		RenderUtils.drawRect(x - 1, head, x + 2, head + 3, color);
		RenderUtils.drawRect(x, head + 3, x + 1, bottom - 1, color);
	}

	/** A pole from near the top to the bottom edge, with a pennant flying to the right of its top. */
	private static void drawFlag(int x, int top, int bottom, int color) {
		final int pole = top + 1;
		RenderUtils.drawRect(x, pole, x + 1, bottom - 1, color);
		RenderUtils.drawRect(x + 1, pole, x + 5, pole + 1, color);
		RenderUtils.drawRect(x + 1, pole + 1, x + 4, pole + 2, color);
		RenderUtils.drawRect(x + 1, pole + 2, x + 3, pole + 3, color);
		RenderUtils.drawRect(x + 1, pole + 3, x + 2, pole + 4, color);
	}

	/** A diamond five across, in the middle of the space. */
	private static void drawDiamond(int x, int top, int bottom, int color) {
		final int middle = (top + bottom) / 2;
		RenderUtils.drawRect(x, middle - 2, x + 1, middle - 1, color);
		RenderUtils.drawRect(x - 1, middle - 1, x + 2, middle, color);
		RenderUtils.drawRect(x - 2, middle, x + 3, middle + 1, color);
		RenderUtils.drawRect(x - 1, middle + 1, x + 2, middle + 2, color);
		RenderUtils.drawRect(x, middle + 2, x + 1, middle + 3, color);
	}
