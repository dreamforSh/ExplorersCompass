...
public final class ConfigTexts {
	...
	public static Component label(ConfigValue<?> value) {
		return translatableWithFallback(PREFIX + keyOf(value), keyOf(value));
	}

	/** The heading a group of settings is shown under. */
	public static Component groupLabel(String groupKey) {
		return translatableWithFallback(PREFIX + "group." + groupKey, groupKey);
	}

	/**
	 * A translation, or the given text where the language files have nothing under the key. Later
	 * versions of the game carry this on the component itself and settle it when the text is drawn;
	 * this one has no such component, so the lookup is settled here instead.
	 */
	...
	public static List<Component> tooltip(ConfigValue<?> value) {
		...
		if (I18n.exists(tooltipKey) || (comment != null && !comment.isBlank())) {
			lines.add(translatableWithFallback(tooltipKey, comment == null ? "" : comment).withStyle((style) -> style.withColor(TextColor.fromRgb(GuiTheme.TEXT_SECONDARY))));
		}
	...
	public static Component enumLabel(Enum<?> constant) {
		return translatableWithFallback(PREFIX + "enum." + constant.getDeclaringClass().getSimpleName() + "." + constant.name().toLowerCase(Locale.ROOT), constant.name());
	}
