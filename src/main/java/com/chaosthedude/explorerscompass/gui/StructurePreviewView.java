...
public class StructurePreviewView implements AutoCloseable {
	...
	public void render(GuiGraphics guiGraphics, StructurePreview preview, int left, int top, int right, int bottom) {
		...
		// Where on the screen the model goes, and the one flip, on the interface's own matrix. That
		// matrix is now a plain stack of the maths library's own rather than a pose stack, so it is
		// pushed and popped by its own names and takes its translation in floats.
		final Matrix4fStack screenStack = RenderSystem.getModelViewStack();
		screenStack.pushMatrix();
		screenStack.translate(left + panelWidth / 2.0F, top + panelHeight / 2.0F, (float) MODEL_DEPTH);
	...
	private static Matrix4f modelViewFor(PoseStack modelStack) {
		// The matrix classes are the maths library's own now, and copying and multiplying are its
		// spellings of both: a copy is a constructor, and multiplying in place is mul
		return new Matrix4f(RenderSystem.getModelViewMatrix()).mul(modelStack.last().pose());
	}
	...
	private void build(StructurePreview preview, int layerLimit) {
		...
		// The memory a build writes into is now held apart from the builder writing into it, and what
		// is being built — the shape of a vertex and how the vertices are grouped — is settled when the
		// builder is made rather than when it is begun. The memory is given back once what was built
		// has been handed to the graphics card.
		try (ByteBufferBuilder buffer = new ByteBufferBuilder(Math.max(MIN_BUFFER_BYTES, drawnCells * bytesPerCell))) {
			final BufferBuilder builder = new BufferBuilder(buffer, VertexFormat.Mode.QUADS,
					simplified ? DefaultVertexFormat.POSITION_COLOR : Sheets.cutoutBlockSheet().format());
			if (simplified) {
			...
			// A buffer now says up front how it is going to be used; this one is built once and only drawn
			geometry = new VertexBuffer(VertexBuffer.Usage.STATIC);
			geometry.bind();
			geometry.upload(mesh);
			VertexBuffer.unbind();
		}
	}
	...
	private static void vertex(BufferBuilder builder, float x, float y, float z, int colour) {
		// A vertex is finished by the call that adds it now, so nothing marks the end of one
		builder.addVertex(x, y, z).setColor((colour >> 16) & 255, (colour >> 8) & 255, colour & 255, 255);
	}
	...
	private Vector3f lightTurnedBack(float x, float y, float z) {
		// Turning a vector by a rotation is the maths library's own rotate, which applies it in place
