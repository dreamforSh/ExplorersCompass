...
public class LayerSlider extends AbstractWidget {
	...
	private int handleTop() {
		return y + Math.round((1.0F - fraction()) * travel());
	}
	...
	public void renderButton(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
		...
		RenderUtils.drawRect(trackLeft, y, trackRight, y + height, TRACK_COLOR);
		RenderUtils.drawInnerOutline(trackLeft, y, trackRight, y + height, TRACK_BORDER);

		// The track fills from the ground up to the handle: what is filled is what is shown
		final int top = handleTop();
		RenderUtils.drawRect(trackLeft + 1, top + HANDLE_HEIGHT / 2, trackRight - 1, y + height - 1, TRACK_FILL);

		final boolean lit = isHoveredOrFocused();
		RenderUtils.drawRect(x, top, x + width, top + HANDLE_HEIGHT, lit ? HANDLE_COLOR_HOVERED : HANDLE_COLOR);
		RenderUtils.drawRect(x + 1, top + 1, x + width - 1, top + 2, 0x60FFFFFF);
		...
		Minecraft.getInstance().font.drawShadow(poseStack, reading, x - 4 - textWidth, textY, lit ? GuiTheme.TEXT_PRIMARY : GuiTheme.TEXT_SECONDARY);
	}
